...
async function listOrgFloorPlans(orgId) {
  const rows = await supabaseRest('org_floor_plans', {
    query: `?select=id,name,venue,ciudad,tipo,width_m,length_m,opacity,created_by_display_name,created_at&organization_id=eq.${encodeURIComponent(orgId)}&order=created_at.desc&limit=500`
  });
  return Array.isArray(rows) ? rows : [];
}
