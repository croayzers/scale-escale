...
async function saveOrgFloorPlan({ orgId, userId, userName, name, venue, ciudad, tipo, cliente, widthM, lengthM, opacity, imageDataUrl }) {
  ...
  // ciudad y tipo están en el schema (01_escale.sql); cliente se añadió en migration 20260613.
  const body = {
    company_id: orgId,
  ...
  // Upsert por (company_id, name): el ?on_conflict dirige el ON CONFLICT al constraint correcto.
  // Sin él, PostgREST usa la PK (id), que nunca coincide y provoca unique violation.
  const upsertHeader = { Prefer: 'resolution=merge-duplicates,return=representation' };
  const upsertQuery  = '?on_conflict=company_id,name';

  // Intentar con cliente; si falla (columna no existe aún) reintentar sin ella
  try {
    const rows = await escaleRest('org_floor_plans', {
      method: 'POST',
      query: upsertQuery,
      headers: upsertHeader,
      body: { ...body, cliente: cliente || null }
    });
    if (Array.isArray(rows) && rows[0]) return rows[0];
  } catch {
    /* columna cliente no existe aún — continuar sin ella */
  }
  const rows = await escaleRest('org_floor_plans', { method: 'POST', query: upsertQuery, headers: upsertHeader, body });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}
...
/* ════════════════════════════════════════════════════════
   QR dinámicos (escale.qr_codes + escale.qr_scan_events)
   Aislamiento por company_id. service-role bypassa RLS, así
   que TODAS estas funciones filtran company_id explícitamente.
   ════════════════════════════════════════════════════════ */

const QR_CODE_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';

function generateQrCode(length = 8) {
  let out = '';
  const bytes = require('crypto').randomBytes(length);
  for (let i = 0; i < length; i++) {
    out += QR_CODE_ALPHABET[bytes[i] % QR_CODE_ALPHABET.length];
  }
  return out;
}

// Genera un code base62 único (reintenta si colisiona contra escale.qr_codes.code)
async function generateUniqueQrCode(maxAttempts = 6) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const code = generateQrCode(attempt < 3 ? 7 : 8);
    const rows = await escaleRest('qr_codes', {
      query: `?select=id&code=eq.${encodeURIComponent(code)}&limit=1`
    });
...
async function createQrCode({ companyId, ownerUserId, type, title, targetUrl, payload, expiresAt }) {
  ...
  return row ? { id: row.id, code: row.code } : { code };
}

async function listQrCodes(companyId) {
  if (!companyId) return [];
  const rows = await escaleRest('qr_codes', {
    query: `?company_id=eq.${encodeURIComponent(companyId)}&kind=eq.dynamic&order=created_at.desc&limit=500` +
           `&select=id,code,kind,type,title,target_url,is_active,expires_at,scan_count,last_scan_at,created_at`
  });
  return Array.isArray(rows) ? rows : [];
}

// Verifica que el QR pertenece a la org antes de devolverlo (NUNCA cross-org).
async function findQrCodeForCompany(qrId, companyId) {
  if (!qrId || !companyId) return null;
  const rows = await escaleRest('qr_codes', {
    query: `?id=eq.${encodeURIComponent(qrId)}&company_id=eq.${encodeURIComponent(companyId)}&limit=1` +
           `&select=id,code,kind,type,title,target_url,payload,is_active,expires_at,scan_count,last_scan_at,created_at`
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}

async function updateQrCode(qrId, companyId, patch) {
  if (!qrId || !companyId) return null;
  // El filtro por company_id en el PATCH garantiza que no se toca un QR de otra org.
  const rows = await escaleRest('qr_codes', {
    method: 'PATCH',
    query: `?id=eq.${encodeURIComponent(qrId)}&company_id=eq.${encodeURIComponent(companyId)}`,
    body: patch
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}

async function listQrScanEvents(qrId) {
  if (!qrId) return [];
  const rows = await escaleRest('qr_scan_events', {
    query: `?qr_id=eq.${encodeURIComponent(qrId)}&order=scanned_at.desc&limit=2000` +
           `&select=id,scanned_at,ip_hash,country,city,device_type,os,browser,referrer,lang,src`
  });
  return Array.isArray(rows) ? rows : [];
}
...
/* ════════════════════════════════════════════════════════
   ORG FILE LIBRARY — Supabase Storage
   Bucket: org-files · Ruta: {orgId}/{uuid}_{filename}
   ════════════════════════════════════════════════════════ */

const ORG_FILES_BUCKET = 'org-files';

async function _storageFetch(endpoint, { method = 'GET', body, binary = false } = {}) {
  const storageBaseUrl = supabaseStorageUrl();
  const key = supabaseServerKey();
  if (!storageBaseUrl || !key) return null;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Storage ${method} ${endpoint}: ${text}`);
  }
  const text = await response.text().catch(() => '');
  return text ? JSON.parse(text) : null;
}

async function listOrgStorageFiles(orgId) {
  if (!orgId) return [];
  const data = await _storageFetch(`/object/list/${ORG_FILES_BUCKET}`, {
    method: 'POST',
    body: { prefix: `${orgId}/`, limit: 500, offset: 0, sortBy: { column: 'created_at', order: 'desc' } }
  });
  return Array.isArray(data) ? data : [];
}

async function createSignedUploadUrl(orgId, filename) {
  if (!orgId || !filename) return null;
  const safe = sanitizeSlug(filename.replace(/\.[^.]+$/, '')) || 'file';
  const ext = String(filename.split('.').pop() || 'bin').toLowerCase().slice(0, 8);
  const uuid = Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
  const path = `${orgId}/${uuid}_${safe}.${ext}`;
  const data = await _storageFetch(`/object/sign/upload/${ORG_FILES_BUCKET}/${path}`, { method: 'POST' });
  if (!data?.signedURL) return null;
  return { signedURL: data.signedURL, path, token: data.token };
}

async function createSignedViewUrl(path, expiresIn = 3600) {
  if (!path) return null;
  const data = await _storageFetch(`/object/sign/${ORG_FILES_BUCKET}/${path}`, {
    method: 'POST',
    body: { expiresIn }
  });
  if (!data?.signedURL) return null;
  const base = supabaseProjectUrl() || supabaseStorageUrl().replace('/storage/v1', '');
  return data.signedURL.startsWith('http') ? data.signedURL : `${base}/storage/v1${data.signedURL}`;
}

async function deleteOrgStorageFile(path) {
  if (!path) return;
  await _storageFetch(`/object/${ORG_FILES_BUCKET}`, {
    method: 'DELETE',
    body: { prefixes: [path] }
  });
}
