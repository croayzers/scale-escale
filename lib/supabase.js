...
const COMPANY_SELECT = 'id,nombre,pais,apps,flags,created_at';
...
function companyToOrg(company, { config = null, sub = null, rol = '' } = {}) {
  ...
  return {
    ...
    logo_path: config?.logo_url || null,
...
async function findSubscription(companyId) {
  ...
  try {
    const rows = await supabaseRest('subscriptions', {
      query: `?select=plan,estado,stripe_customer_id,stripe_subscription_id&company_id=eq.${companyId}&limit=1`
    });
    return Array.isArray(rows) && rows[0] ? rows[0] : null;
  } catch { return null; }
}
...
async function listUserMemberships(userId) {
  ...
  const rows = await supabaseRest('company_members', {
    query: `?select=company_id,rol,companies(${COMPANY_SELECT})&user_id=eq.${userId}`
  });
  return (Array.isArray(rows) ? rows : [])
    ...
    .map(r => ({
      ...
      role: r.rol,
      status: 'active',
      organization: companyToOrg(r.companies, { rol: r.rol })
    }));
}
...
async function findOwnedOrganizations(userId) {
  if (!userId) return [];
  const rows = await supabaseRest('company_members', {
    query: `?select=company_id,companies(${COMPANY_SELECT})&user_id=eq.${userId}&rol=eq.owner&limit=8`
  });
  return (Array.isArray(rows) ? rows : [])
    .filter(r => r.companies?.id)
    .map(r => companyToOrg(r.companies, { rol: 'owner' }));
}
...
async function syncOrganizationProfile(companyId, company = {}) {
  ...
  let logoUrl = organization.logo_path || null;
  ...
  await escaleRest('empresa_config', {
    method: 'POST',
    headers: { Prefer: 'resolution=merge-duplicates,return=representation' },
    body: { company_id: companyId, billing_email: billingEmail, venue_default: cleanText(company.venueName) || organization.venue_default || null, logo_url: logoUrl }
  });

  return { ...organization, nombre, billing_email: billingEmail, logo_path: logoUrl };
}
...
async function findBillingCustomer(companyId) {
  ...
  try {
    const rows = await supabaseRest('subscriptions', {
      query: `?select=company_id,plan,estado,stripe_customer_id,stripe_subscription_id&company_id=eq.${companyId}&limit=1`
    });
    return Array.isArray(rows) && rows[0] ? rows[0] : null;
  } catch { return null; }
}
